...
func TestInLocation(t *testing.T) {
	...
	time.Local = nil
	ts, err = Strtotime("2013-02-01 00:00:00", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	ts, err = Strtotime("18 January 2018", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	ts, err = Strtotime("2013-04-01 00:00:00", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	// Now some errors
	zeroTime := time.Time{}.Unix()
	ts, err = Strtotime("INVALID", denverLoc)
	assert.Equal(t, zeroTime, ts.Unix())
	assert.NotEqual(t, nil, err)
