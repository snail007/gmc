...
func TestParse(t *testing.T) {
	...
	_, err = StrToTime("138421636711122233311111") // too many digits
	assert.NotEqual(t, nil, err)

	_, err = StrToTime("-1314")
	assert.NotEqual(t, nil, err)

	_, err = StrToTime("2014-13-13 08:20:13,787") // month 13 doesn't exist so error
	assert.NotEqual(t, nil, err)
}

func testDidPanic(datestr string) (paniced bool) {
	t := MustStrToTime(datestr)
	if t.IsZero() {
		paniced = true
		return
	}
...
func TestInLocation(t *testing.T) {
	...
	ts := MustStrToTime("2013-02-01 00:00:00")
	zone, offset := ts.Zone()
	...
	ts = MustStrToTime("Tue, 5 Jul 2017 16:28:13 -0700 (MST)")
	...
	time.Local = nil
	ts, err = StrToTimeIn("2013-02-01 00:00:00", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	ts, err = StrToTimeIn("18 January 2018", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	ts, err = StrToTimeIn("2013-04-01 00:00:00", denverLoc)
	assert.Equal(t, nil, err)
	zone, offset = ts.Zone()
	...
	//   UnixDate    = "Mon Jan _2 15:04:05 MST 2006"
	ts = MustStrToTime("Mon Jan  2 15:04:05 MST 2006")
	...
	// RFC850    = "Monday, 02-Jan-06 15:04:05 MST"
	ts = MustStrToTime("Monday, 02-Jan-06 15:04:05 MST")
	_, offset = ts.Zone()
	assert.Equal(t, 0, offset, "Should have found offset = 0 %v", offset)
	assert.Equal(t, "2006-01-02 15:04:05 +0000 UTC", fmt.Sprintf("%v", ts.In(time.UTC)))
	...
	// Now some errors
	zeroTime := time.Time{}.Unix()
	ts, err = StrToTimeIn("INVALID", denverLoc)
	assert.Equal(t, zeroTime, ts.Unix())
	assert.NotEqual(t, nil, err)
