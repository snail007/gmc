...
# 配置文件
gmc使用toml作为配置文件，配置文件里面配置，可以通过设置对应环境变量动态改变。
...
listen=":6800"
```
那么可以通过设置环境变量`GMC_HTTPSERVER_LISTEN=:9999`，来改变程序监听的http服务端口为`9999`。

可以发现，规则如下：

1. 环境变量由`GMC_`前缀加上对应配置里面的配置名称的全部大写组成。
1. 配置文件里面的层级关系，在环境变量里面通过下划线`_`表示。

另外：

`GMC_`这个前缀是可以通过设置环境变量`ENV_PREFIX`来改变的.

比如：

设置环境变量`ENV_PREFIX=APP_`，把前缀改成`APP_`,那么设置环境变量:`APP_HTTPSERVER_LISTEN=:9999`，就能改变程序监听的http服务端口为`9999`。
...
针对生成的管理后台，GMCT 同时可以对应生成操作表数据的视图文件，列表数据，增加修改数据，免去了手写views的麻烦。

下面的命令需要在views的具体子目录下面执行。
...
-n 是控制器在路由中注册的名称，比如表名称：`user`, `sysconfig`。

-t 是数据库表的名称，如果配置文件里面设置了表前缀，这里不需要写表前缀。
...
GMCT 可以生成一个有若干通用操作表数据的方法的表模型文件，免去了手写表模型的麻烦。
